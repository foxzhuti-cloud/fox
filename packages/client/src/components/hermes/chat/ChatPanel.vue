...
import { renameSession, setSessionWorkspace, batchDeleteSessions, exportSession, fetchSession } from "@/api/hermes/sessions";
...
async function handleContextMenuSelect(key: string) {
  ...
  if (key === "copy-id") {
    ...
    await handleExportMarkdown(contextSessionId.value);
  } else if (key === "export-pdf") {
    await handleExportPdf(contextSessionId.value);
...
async function handleExportMarkdown(sessionId: string) {
  ...
  const loadingMsg = message.loading(t("chat.exportFetchingMessages"), { duration: 0 });
  try {
    const detail = await fetchSession(sessionId);
    loadingMsg.destroy();
    if (!detail || !detail.messages || detail.messages.length === 0) {
      message.warning(t("chat.sessionNotFound"));
      return;
    }
    const markdown = messagesToMarkdown(detail.messages, {
...
async function handleExportPdf(sessionId: string) {
  ...
  try {
    ...
    openPdfPrint(markdown, session?.title || 'Conversation');
  } catch {
    loadingMsg.destroy();
    message.error(t("chat.exportFailed"));
  }
